...
export default function Results() {
  return (
    <>
      <section
        id="results"
        className="relative overflow-hidden bg-gradient-to-br from-white via-blue-50/40 to-cyan-50/30 px-6 py-32"
      >
        {/* ================= BACKGROUND ================= */}
        <div className="absolute inset-0 -z-10 overflow-hidden">
          <div className="absolute left-1/2 top-0 h-[550px] w-[550px] -translate-x-1/2 rounded-full bg-blue-200/20 blur-3xl" />
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <motion.div
            ...
            className="max-w-3xl"
          >
            <div className="inline-flex items-center gap-2 rounded-full border border-blue-100 bg-white/80 px-4 py-2 text-sm font-medium text-blue-600 shadow-sm backdrop-blur-sm">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            className="mt-16 grid gap-6 rounded-[36px] border border-black/5 bg-white/70 p-8 shadow-[0_20px_60px_-20px_rgba(0,0,0,0.08)] backdrop-blur-xl md:grid-cols-4"
          >
            {[
              { value: "$5K+", label: "Advertising Spend Managed" },
              { value: "+22%", label: "ROAS Improvement" },
              { value: "+18%", label: "Conversion Rate Increase" },
              { value: "100+", label: "Users Acquired" },
            ].map((stat) => (
            ...
            {results.map((result, index) => {
              ...
              return (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  whileHover={{ y: -10 }}
                  className="group relative overflow-hidden rounded-[32px] border border-black/5 bg-white/80 p-8 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.08)] backdrop-blur-xl"
                >
                  <div
                    className={`absolute inset-0 opacity-0 blur-3xl transition-opacity duration-500 group-hover:opacity-20 bg-gradient-to-br ${result.gradient}`}
                  />
                  <div className="absolute inset-0 rounded-[32px] border border-transparent transition-all duration-300 group-hover:border-blue-100" />

                  <div className="relative">
                    <div className="flex items-start justify-between">
                      <div
                        className={`flex h-15 w-15 items-center justify-center rounded-2xl bg-gradient-to-br ${result.gradient} shadow-lg`}
                      >
                        <Icon className="h-7 w-7 text-white" />
                      </div>
                      <ArrowUpRight className="h-5 w-5 text-zinc-300 transition-all duration-300 group-hover:translate-x-1 group-hover:-translate-y-1 group-hover:text-black" />
                    </div>
          ...
          {/* ================= POST-PROOF CTA ================= */}
          {/* This is the highest-converting spot — social proof immediately precedes the ask */}
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.2 }}
            viewport={{ once: true }}
            className="mt-16 flex flex-col items-center gap-4 rounded-[32px] border border-blue-100 bg-white/80 px-8 py-12 text-center shadow-lg backdrop-blur-xl"
          >
            <p className="text-sm font-semibold uppercase tracking-widest text-blue-500">
              Want results like these?
            </p>
            <h3 className="max-w-xl text-3xl font-bold text-black md:text-4xl">
              Get a free audit of your website — delivered as a personal Loom
              video
            </h3>
            <p className="max-w-lg text-zinc-500">
              I&apos;ll personally review your site and record a video
              walkthrough covering what&apos;s working, what&apos;s costing you
              conversions, and exactly what I&apos;d fix first.
            </p>
            ...
            <p className="text-xs text-zinc-400">
              No commitment. Audit delivered within 24 hours.
            </p>
          </motion.div>
        </div>
      </section>
