...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        <div className="mx-auto flex h-18 max-w-7xl items-center justify-between rounded-2xl border border-white/60 bg-white/90 px-6 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.12)] backdrop-blur-xl">
          {/* Logo */}
            ...
            <div className="flex items-center gap-1 rounded-full border border-black/5 bg-white/70 p-1 shadow-sm">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className="rounded-full px-5 py-2 text-sm font-medium text-zinc-600 transition-all hover:bg-blue-50 hover:text-blue-600"
                >
                  {link.name}
                </a>
              ))}
            ...
            <Sheet>
              ...
              >
                <div className="mt-8 flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-black">Convertify</h3>
                ...
                <div className="mt-12 flex flex-col gap-3">
                  {navLinks.map((link) => (
                    <a
                      key={link.name}
                      href={link.href}
                      className="rounded-2xl border border-black/5 bg-white px-5 py-4 text-base font-medium text-zinc-700 hover:border-blue-200 hover:bg-blue-50 hover:text-blue-600"
                    >
                      {link.name}
                    </a>
                  ))}
